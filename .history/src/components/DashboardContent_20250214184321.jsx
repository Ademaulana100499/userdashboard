...
const DashboardContent = () => {
  const [sortConfig, setSortConfig] = useState({
    key: "username",
    direction: "asc",
    clicked: false, // Menandakan jika sudah diklik
  });
  ...
  const requestSort = (key) => {
    if (sortConfig.clicked) return; // Jika sudah diklik, tidak bisa klik lagi

    let direction = "asc";
    if (sortConfig.key === key && sortConfig.direction === "asc") {
      direction = "desc";
    }
    setSortConfig({ key, direction, clicked: true }); // Set clicked menjadi true setelah klik pertama
  };
            ...
            <th className="border border-slate-300 px-4 py-2 text-left">
              ...
              <div className="flex flex-col items-center justify-center space-y-1">
                <FiChevronUp
                  ...
                  disabled={sortConfig.clicked} // Menonaktifkan klik setelah satu klik
                />
                <FiChevronDown
                  className={`text-xs ${
                    sortConfig.key === "username" &&
