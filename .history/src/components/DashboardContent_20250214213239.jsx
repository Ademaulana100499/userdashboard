...
const DashboardContent = () => {
  ...
  const [keyword, setKeyword] = useState(""); // Track search keyword

  const fetchData = async (gender, keyword) => {
    try {
      const response = await fetch(
        `https://randomuser.me/api/?page=1&pageSize=10&results=10&gender=${gender}&keyword=${keyword}`
      );
  ...
  useEffect(() => {
    fetchData(selectedGender, keyword); // Fetch data with gender and keyword
  }, [selectedGender, keyword]);
      ...
      <div>
        ...
        <div className="flex items-center">
          ...
          <button
            ...
            onClick={() => fetchData(selectedGender, keyword)} // Trigger fetch with current keyword
          >
            <FiSearch />
          </button>
        </div>
